    ...
    <div ng-controller="menuController" class="row">
        ...
        <div  style="width:100%;" class="card-box">
            <table class="table table-hover bls-adm-menu">
                ...
                <tbody>
                <tr ng-class="{'bg-primary': item.id == Menu.selectedItem.id, 'level-@{{item.lavel}}' : item.level > 0}"
                    ng-model="Menu.items"
                    ng-repeat="item in Menu.items | orderBy: '+sort'">
                    <th>
                    ...
                    <td>
                        <input type="number" class="form-control col-xs-2" ng-blur="chengeMenuSort(item, $event)" value="@{{item.sort}}">
                    </td>
                    <td>
                        <input type="text" class="form-control col-xs-2" ng-model="item.url" value="@{{item.url}}">
                    </td>
                    <td>
                        <input type="text" class="form-control col-xs-2" ng-model="item.icon" value="@{{item.icon}}">
                    </td>
                    <td>
